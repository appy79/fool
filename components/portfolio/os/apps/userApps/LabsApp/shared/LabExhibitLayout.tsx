...
export default function LabExhibitLayout({
  ...
  signals,
  proof,
  gridClassName = "@3xl:grid-cols-[1.35fr_0.85fr]",
  ...
  return (
    <section id={id} className="scroll-mt-24 text-foreground">
      <div className="min-w-0 space-y-6">
        <header className="grid min-w-0 gap-5 border-b border-border/70 pb-6 @2xl:grid-cols-[minmax(0,1fr)_minmax(15rem,0.55fr)] @2xl:items-start">
          <div className="min-w-0 font-mono">
            ...
            <h1 className="mt-3 min-w-0 break-words text-2xl font-semibold tracking-[-0.03em] text-foreground [overflow-wrap:anywhere] @lg:text-3xl">
              {programTitle}
            </h1>
          </div>

          <div className="min-w-0 border-t border-border/70 pt-4 @2xl:border-l @2xl:border-t-0 @2xl:pl-5 @2xl:pt-0">
            {moduleSelector}
            <div className="mt-3 space-y-1 text-sm leading-6 text-muted-foreground">
              <p className="break-words font-medium text-foreground">
                loaded module: {moduleTitle}
              </p>
            </div>
          </div>
        </header>
