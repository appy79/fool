...
export default function TuringTapeScene({
  ...
  const visibleCells = Array.from(
    { length: tapeRadius * 2 + 1 },
    (_, index) => head - tapeRadius + index,
  );
      ...
      >
        ...
        <div className="mb-4 flex items-center justify-between gap-3 rounded-2xl border border-amber-100/10 bg-black/25 px-4 py-3 relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 font-mono text-xs uppercase tracking-[0.22em]">
            ...
            <div className="flex gap-1.5 items-center">
              <span className="text-[0.55rem] text-amber-400/80 font-bold uppercase tracking-widest">
                VACUUM GLOW:
              </span>
              <div
                className="h-6 w-3.5 rounded-t-full rounded-b-sm border border-red-500/50 bg-gradient-to-b from-red-500/80 to-stone-900"
                style={{
                  animation: machineVisualActive ? "turing-valve-glow 1.5s infinite" : "none",
                }}
              />
            </div>

            <span
              className={`h-3 w-3 rounded-full ${machineVisualActive ? "animate-pulse bg-emerald-300 motion-reduce:animate-none" : "bg-amber-900"}`}
            />
            {clackTick > 0 ? (
              <span
                key={`clack-label-${clackTick}`}
                className="rounded-full border border-amber-100/20 bg-amber-100/10 px-3 py-1 text-amber-100 animate-pulse motion-reduce:animate-none"
              >
                {lastMove === "S" ? "CLACK" : lastMove === "R" ? "CLICK ->" : "<- CLACK"}
              </span>
            ) : (
              <span className="rounded-full border border-amber-100/10 px-3 py-1 text-amber-200/60">
                IDLE
              </span>
            )}
          </div>
        ...
        <div className="relative h-[336px] overflow-hidden rounded-[1.5rem] border border-amber-100/10 bg-[radial-gradient(circle_at_50%_35%,rgba(245,158,11,0.14),transparent_34%),linear-gradient(180deg,#332315,#120d08)]">
            ...
            <div className="absolute bottom-2 left-0 right-0 flex justify-around">
              {Array.from({ length: 34 }).map((_, index) => (
                <span
                  ...
                  className="h-2 w-2 rounded-full bg-[#5c3a16]/65"
                />
              ))}
            </div>
          ...
          <div className="absolute bottom-5 left-5 right-5 z-20 grid grid-cols-3 gap-3 rounded-2xl border border-amber-100/10 bg-black/30 p-3 font-mono text-xs text-amber-100/80">
            ...
            <span>
              WRITE:{" "}
              {lastTransition.includes("write")
                ? lastTransition.split("write ")[1]?.split(",")[0]
                : "-"}
            </span>
            <span>
