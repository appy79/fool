...
export default function useComplexityLab() {
  ...
  const scenario =
    complexityScenarios.find((item) => item.id === scenarioId) ?? complexityScenarios[0];
  const {
    isRunning,
  ...
  const activeCaption = isRouteSearch
    ? routeCaptions[activeDepth]
    : constraintCaptions[activeDepth];
