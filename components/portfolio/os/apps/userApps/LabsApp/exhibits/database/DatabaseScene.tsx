...
function stageClass(activeStage: number, index: number) {
  ...
  return cx(
    ...
    state === "active" &&
      "scale-105 border-emerald-300 bg-emerald-400 text-zinc-950 shadow-lg shadow-emerald-500/25",
...
function nodeClass(
  active: boolean,
  complete = false,
  tone: "emerald" | "amber" | "sky" | "violet" | "rose" = "emerald",
) {
  const activeClasses = {
        ...
        {stages.map((stage, index) => (
          <div key={stage} className="flex min-w-0 flex-1 items-center gap-2">
            ...
            {index < stages.length - 1 ? (
              <div className={connectorClass(activeStage >= index + 1, activeStage > index + 1)} />
            ) : null}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="grid h-full grid-cols-[0.9fr_1.4fr_0.9fr] gap-5">
      <div className="flex flex-col justify-center gap-3">
        ...
        <div className={nodeClass(activeStage === 1, activeStage > 1, "amber")}>
      ...
      <div className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-4">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-[0.62rem] font-black uppercase tracking-[0.2em] text-rose-300">
            Buffer Pool And Heap Pages
          </span>
          <span className="rounded-full border border-rose-400/30 px-2 py-1 text-[0.56rem] font-black uppercase tracking-widest text-rose-200">
            O(n)
          </span>
        </div>
        <div className="grid grid-cols-3 gap-3">
          {pages.map((page, index) => {
            ...
            const touched =
              activeStage > 2 ||
              (readingPages && index <= Math.min(activeStage + 1, pages.length - 1));
            ...
            return (
              <div
                ...
                className={cx(
                  ...
                  readingPages &&
                    touched &&
                    "scale-105 border-rose-300 bg-rose-500/20 text-rose-100 shadow-lg shadow-rose-500/20",
              ...
              >
                ...
                <p className="mt-1 text-[0.55rem] uppercase tracking-widest opacity-70">
                  {filtered ? "match kept" : touched ? "tuple scan" : "waiting"}
                </p>
              </div>
            );
          })}
...
function IndexedVisual({ activeStage }: { activeStage: number }) {
  return (
    <div className="grid h-full grid-cols-[1.05fr_1.35fr_0.95fr] gap-5">
      <div className="flex flex-col justify-center gap-3">
        <div className={nodeClass(activeStage === 0, activeStage > 0, "sky")}>
          Indexed predicate
        </div>
        <div className={connectorClass(activeStage >= 1, activeStage > 1)} />
      ...
      <div className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-4">
        <div className="mb-3 text-[0.62rem] font-black uppercase tracking-[0.2em] text-amber-300">
          B-tree Traversal
        </div>
      ...
      <div className="flex flex-col justify-center gap-3">
        <div className={nodeClass(activeStage === 2, activeStage > 2, "emerald")}>
          Heap pointer fetch
        </div>
...
function ParallelVisual({ activeStage }: { activeStage: number }) {
  return (
    <div className="grid h-full grid-cols-[0.85fr_1.55fr_0.9fr] gap-5">
      ...
      <div className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-4">
        <div className="mb-3 text-[0.62rem] font-black uppercase tracking-[0.2em] text-sky-300">
          Worker Lanes
        </div>
        <div className="grid grid-cols-3 gap-3">
          {["Worker 1\nblocks 0-33", "Worker 2\nblocks 34-66", "Worker 3\nblocks 67-99"].map(
            (label, index) => (
              <div
                key={label}
                className={cx(
                  nodeClass(activeStage === 2, activeStage > 2, "sky"),
                ...
                <div className="mt-3 h-2 overflow-hidden rounded-full bg-zinc-800">
                  <div
                    className={cx(
                      ...
                      activeStage >= 2 ? "w-full bg-sky-300" : "w-0 bg-sky-300",
                    )}
                  />
                </div>
    ...
    <div className="grid h-full grid-cols-[0.95fr_1.25fr_1fr] gap-5">
      ...
      <div className="rounded-3xl border border-emerald-500/30 bg-emerald-950/20 p-5">
        <div className="mb-4 flex items-center justify-between">
          <span className="text-[0.62rem] font-black uppercase tracking-[0.2em] text-emerald-300">
            Read-Through Cache Fill
          </span>
          <span className="rounded-full border border-emerald-300/40 px-2 py-1 text-[0.56rem] font-black uppercase tracking-widest text-emerald-200">
            next read is hot
          </span>
        </div>
        <div className={cx(nodeClass(activeStage === 3, activeStage > 3, "emerald"), "min-h-24")}>
          Populate Redis
          <span className="mt-2 block text-[0.55rem] tracking-widest opacity-75">
            set key + TTL after DB fetch
          </span>
        </div>
      ...
      <div className="flex flex-col justify-center gap-3">
        <div className={nodeClass(activeStage === 2, activeStage > 2, "rose")}>
          Primary DB fetch
        </div>
