...
export const databaseAccessModules = [
    ...
    insightSteps: [
      {
        ...
        description:
          "The incoming request is parsed into an abstract syntax tree and prepared for planning.",
      },
      {
        title: "Planner initiates full relation scan",
        description:
          "Without selective statistics or an index, the planner resorts to a full table scan.",
      },
      {
        title: "Disk pages loaded into buffer pool",
        description:
          "Every page in the relation must be read from disk or cache buffers, increasing I/O churn.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Each row is tested against the query filter predicates to select matching data.",
      },
      {
        title: "Matching rows returned to client",
        description:
          "The result is compiled and returned, completing the slow, baseline disk-heavy loop.",
      },
    ],
  },
  {
    ...
    insightSteps: [
      ...
      },
      {
        ...
        description:
          "The index is searched via rapid logarithmic leaf lookups, bypassing almost all data pages.",
      },
      {
  ...
  },
  {
    ...
    summary:
      "Independent partitions run concurrently, improving throughput until merge or lock contention appears.",
    stages: [
      "Query Request",
      "Partition Split",
      "Parallel Workers",
    ...
    insightSteps: [
      {
        ...
        description:
          "The database engine identifies that a parallel scan of the relation will be cheaper.",
      },
      {
        title: "Divide relation into table blocks",
        description:
          "The planner chunks the table into independent block ranges for parallel execution.",
      },
      {
        title: "Multiple background worker threads scan",
        description:
          "Concurrent CPU cores scan different segments of the data stream simultaneously.",
      },
      {
        title: "Gather and merge worker intermediate sets",
        description:
          "The coordinator gathers and merges row segments from all workers, creating a bottleneck.",
      },
      {
  ...
  },
  {
    ...
    trigger: "Warm cache",
    summary:
      "A miss falls through to the database, then the result is written into Redis so later reads take the fast path.",
    stages: [
      "Query Request",
      "Cache Miss",
      "Database Fetch",
    ...
    insightSteps: [
      ...
      },
      {
        ...
        description:
          "The key is not in memory yet, so the read-through path must fall through to the primary database.",
      },
      {
        title: "Primary database fetches source row",
        description:
          "The application loads the authoritative value from the slower persistence layer.",
      },
      {
        title: "Result is written into Redis",
        description:
          "The fetched payload is cached with a key and TTL so repeat reads can bypass the database.",
      },
      {
        title: "Response returns from warmed path",
        description:
          "The first request pays the fill cost, while later requests can return from Redis memory.",
      },
    ],
  },
  ...
  steps: [
    {
      ...
      description:
        "The first stage establishes work entering the scan, index, parallel worker, or cache module.",
    },
    {
      title: "Planner or keying decision activates",
      description:
        "Planner, sharding, or key hashing shows how the database chooses where to spend coordination effort.",
    },
    {
      title: "Data access strategy changes cost",
      description:
        "Index, scan, cache, or worker execution shifts the balance between latency and resources.",
    },
    {
      title: "Response exposes the tradeoff",
      description:
        "The final stage and bars show whether latency, memory, or throughput improved or regressed.",
    },
  ],
  concepts: [
    {
      ...
      description:
        "Database performance depends on how queries reach data, not just on faster code around the query.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "A database fix is only meaningful when it targets the actual limiting resource.",
      bullets: [
