...
export const telecomStages = [
  {
    ...
    layer: "Device",
    description:
      "A subscriber device starts a network session whose usage may later become chargeable records.",
  ...
  },
  {
    ...
    layer: "Charging",
    description:
      "Charging functions collect, correlate, and prepare charging records for rating, mediation, or billing.",
...
export const telecomScenarios = [
  {
    ...
    summary:
      "Follow a data session from access and core validation into charging, mediation, backend persistence, and billing.",
    ...
    insightSteps: [
      {
        ...
        description:
          "The subscriber device starts a data session. The chargeable usage is observed and prepared later in the core and charging path.",
      },
      {
        title: "RAN forwards bearer context",
        description:
          "Radio access carries the data-session context from the device into the mobile network.",
      },
      {
        title: "Core validates session state",
        description:
          "The control plane checks subscriber state and prepares policy inputs for the charging decision.",
      },
      {
        title: "Policy checks entitlement and quota",
        description:
          "Policy rules decide whether the session is allowed and which charging behavior applies.",
      },
      {
        title: "Charging prepares the record",
        description:
          "Charging functions produce charging records and rating context. Final pricing may happen here or in downstream billing, depending on the operator stack.",
      },
      {
        title: "Kafka decouples event processing",
        description:
          "The charging record enters a mediated stream so downstream services can scale independently.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Persistent storage captures the rated usage and read models needed by product systems.",
      },
      {
        title: "Billing receives the outcome",
        description:
          "The final hop completes the path from network activity to a billing-facing result.",
      },
    ],
  },
  {
    ...
    summary:
      "Watch already-produced charging records move through mediation, aggregation, checkpointing, and billing sync.",
    ...
    stageOverrides: {
      charging: {
        ...
        layer: "Mediation",
        description:
          "The batch job starts from charging records that have already been collected or mediated upstream.",
    ...
    guardrail: {
      ...
      stageId: "store",
      description:
        "Batch checkpoints let workers retry a failed billing sync without replaying the entire day.",
    },
    insightSteps: [
      {
        title: "Charging records enter aggregation",
        description:
          "The flow starts from CDR/EDR or mediated charging records, not from the live UE/RAN session.",
      },
      {
        title: "Kafka absorbs daily volume",
        description:
          "The stream keeps ingestion from blocking downstream billing sync and service processing.",
      },
      {
        title: "Services reconcile billing state",
        description:
          "Application services group records, normalize legacy boundaries, and prepare a consistent billing view.",
      },
      {
        title: "Store protects the aggregate",
        description:
          "Durable storage captures the aggregate before it is synchronized into billing.",
      },
      {
        title: "Billing sync completes",
        description:
          "The final hop represents faster downstream synchronization with reliability protected by storage.",
      },
    ],
  },
  {
    ...
    summary:
      "Simulate an OSS/BSS service order moving through orchestration, audit streaming, durable state, and billing updates.",
    ...
    stageOverrides: {
      ue: {
        ...
        layer: "OSS/BSS",
        description:
          "An order-management, CRM, or API entry point starts the service activation request.",
        signal: "Service order",
      },
      services: {
        label: "Orchestrator",
        description:
          "Application services coordinate fulfillment across inventory, network-facing APIs, and downstream systems.",
    ...
    guardrail: {
      ...
      stageId: "kafka",
      description:
        "Every activation emits an audit event so downstream recovery can reconstruct the service state.",
    },
    insightSteps: [
      {
        title: "Order API receives activation",
        description:
          "Provisioning begins in OSS/BSS or an external API, not in the live RAN session path.",
      },
      {
        title: "Services orchestrate fulfillment",
        description:
          "Application services coordinate the integrated platform behavior and call any required network-facing systems.",
      },
      {
        title: "Kafka publishes platform events",
        description:
          "Provisioning outcomes are streamed so audit, billing, and downstream systems can react.",
      },
      {
        title: "Store records the activation",
        description:
          "Durable state captures what was provisioned and what downstream consumers should see.",
      },
      {
        title: "Billing closes the lifecycle",
        description:
          "The completed path connects activation state to subscription, invoicing, or revenue-facing systems.",
      },
    ],
  },
  ...
  steps: [
    {
      ...
      description:
        "The flow starts from either a live network session or an OSS/BSS order, depending on the scenario.",
    },
    {
      title: "Context and entitlement resolve",
      description:
        "Network control-plane functions or backend orchestration resolve subscriber context, entitlement, quota, and charging inputs.",
    },
    {
      title: "Charging or orchestration emits events",
      description:
        "Charging records, provisioning outcomes, or billing batches are published into the streaming path.",
    },
    {
      title: "Services persist and reconcile outcome",
      description:
        "Microservices enrich the record, write read models, and complete the billing-facing result.",
    },
  ],
  concepts: [
    {
      ...
      description:
        "Telecom platforms turn network activity or OSS/BSS orders into charging records, activation state, and durable billing outcomes.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "High-volume charging systems rely on asynchronous streams and bounded microservice responsibilities.",
      bullets: [
