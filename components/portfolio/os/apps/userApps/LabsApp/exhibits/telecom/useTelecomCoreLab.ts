...
export default function useTelecomCoreLab() {
  ...
  const scenario: TelecomScenario =
    telecomScenarios.find((item) => item.id === scenarioId) ?? telecomScenarios[0];
  const {
  ...
  const baseActiveStage =
    telecomStages.find((stage) => stage.id === activeStageId) ?? telecomStages[0];
  const activeStage = { ...baseActiveStage, ...scenario.stageOverrides?.[baseActiveStage.id] };
  const isComplete =
    activeIndex === scenario.route.length - 1 && !isSimulating && eventsProcessed > 0;
  ...
  const guardrailStageIndex = scenario.guardrail
    ? scenario.route.indexOf(scenario.guardrail.stageId)
    : -1;
  const guardrailReached = guardrailStageIndex >= 0 && activeIndex >= guardrailStageIndex;
  const guardrailPosition = scenario.guardrail
    ? telecomStagePositions[scenario.guardrail.stageId]
    : undefined;
  const insightStepIndex = mapPhaseToInsightStep(
    activeIndex,
    scenario.route.length,
  ...
  const selectStep = (stepIndex: number) => {
    const routeIndex = mapInsightStepToPhase(
      stepIndex,
      activeInsight.steps.length,
      scenario.route.length,
    );
