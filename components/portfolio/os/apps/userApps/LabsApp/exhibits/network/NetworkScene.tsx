  ...
  return (
    <LabSceneFrame className="p-0">
      ...
      >
        ...
        {scenarioId === "edge-hit" ? (
          <div
            className={`absolute left-[50%] top-[9%] rounded-full border border-emerald-400 bg-emerald-950/90 px-4 py-1 text-center text-xs font-black uppercase tracking-[0.18em] text-emerald-100 shadow-lg shadow-emerald-500/20 transition ${activeIndex >= 3 ? "opacity-100" : "opacity-35"}`}
          >
        ...
        {scenarioId === "origin-miss" ? (
          <div
            className={`absolute left-[64%] top-[11%] rounded-full border border-orange-400 bg-orange-950/90 px-4 py-1 text-center text-xs font-black uppercase tracking-[0.18em] text-orange-100 shadow-lg shadow-orange-500/20 transition ${activeIndex >= 3 ? "opacity-100" : "opacity-35"}`}
          >
        ...
        {scenarioId === "api-edge" ? (
          <div
            className={`absolute left-[46%] top-[72%] rounded-full border border-violet-400 bg-violet-950/90 px-4 py-1 text-center text-xs font-black uppercase tracking-[0.18em] text-violet-100 shadow-lg shadow-violet-500/20 transition ${activeIndex >= 2 ? "opacity-100" : "opacity-35"}`}
          >
            🔒 TLS/L7 Edge Handling
          </div>
        ) : null}
        <div
          className={`absolute left-[17%] top-[27%] rounded-full border border-sky-300/30 bg-sky-950/80 px-3 py-1 text-[0.58rem] font-black uppercase tracking-[0.16em] text-sky-100 transition ${activeIndex >= 1 ? "opacity-100" : "opacity-35"}`}
        >
        ...
        {route.slice(0, -1).map((fromId, index) => {
          ...
          return (
            <SceneConnector
              ...
              className={
                completed ? "bg-cyan-300 shadow-[0_0_12px_rgba(103,232,249,0.55)]" : "bg-slate-800"
              }
              from={from}
        ...
        {Object.entries(networkNodePositions).map(([nodeId, node]) => {
          ...
          if (nodeId === "client") {
            ...
            shapeClasses =
              "rounded-t-3xl rounded-b-xl border-t-[4px] border-cyan-400 bg-[#092e40]/85 px-4";
            prefixLabel = "☁️ ";
          } else if (nodeId === "origin") {
            // Solid cylindrical server vault
            shapeClasses =
              "rounded-b-2xl rounded-t-[1.1rem] border-t-8 border-t-slate-500 bg-slate-900/90";
            prefixLabel = "🗄️ ";
          } else if (nodeId === "fill") {
            ...
            shapeClasses =
              "rounded-xl border-y-4 border-l border-r border-indigo-400 bg-[#16122d]/90";
            prefixLabel = "⚙️ ";
          ...
          return (
            ...
            >
              {active ? (
                <div className="absolute inset-[-0.65rem] animate-ping rounded-2xl bg-sky-300/20 motion-reduce:animate-none" />
              ) : null}
              <div
              ...
              >
                <div className="flex flex-col items-center justify-center">
                  <span className="text-[0.62rem] font-bold opacity-60 uppercase tracking-widest leading-none mb-0.5">
                    {prefixLabel}
                    {nodeId}
                  </span>
