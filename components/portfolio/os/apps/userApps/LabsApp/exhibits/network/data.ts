...
export const networkScenarios = [
  {
    ...
    summary:
      "DNS resolves the user to a nearby POP, where the edge cache serves the asset without origin traffic.",
    ...
    insightSteps: [
      {
        ...
        description:
          "The browser begins with a normal asset request before any cache decision is known.",
      },
      {
        title: "DNS chooses a nearby edge",
        description:
          "DNS returns an address for a close point of presence; it does not carry the HTTP payload.",
      },
      {
        title: "Edge handles the request",
        description:
          "After resolution, the browser sends the request to the nearby edge POP, keeping the round-trip path short.",
      },
      {
        title: "Edge cache answers locally",
        description:
          "The cached object is returned from the POP, bypassing origin and app infrastructure.",
      },
      {
        title: "Response returns from the POP",
        description:
          "The user receives the asset from the edge cache, so origin load stays at zero.",
      },
    ],
  },
  {
    ...
    summary:
      "A cache miss travels from edge to origin, fills CDN storage, then returns to the client.",
    ...
    insightSteps: [
      ...
      },
      {
        ...
        description:
          "DNS still returns a nearby POP address before the HTTP request checks cache state.",
      },
      {
        title: "Edge cache lookup misses",
        description:
          "The POP checks its cache and discovers that the object cannot be served locally yet.",
      },
      {
        title: "CDN miss continues to origin",
        description:
          "The route extends beyond the fast path to the origin/object store, increasing latency and origin load.",
      },
      {
        title: "Origin retrieves the cold object",
        description:
          "The origin fetch supplies the missing asset and creates the opportunity to fill cache storage.",
      },
      {
        title: "Cache fills on the response path",
        description:
          "The response writes the object into edge/CDN storage so future requests can become hits.",
      },
      {
        title: "Client receives the fetched object",
        description:
          "The first cold request is slower, but the path has warmed the cache for the next user.",
      },
    ],
  },
  {
    ...
    trigger: "Call API",
    summary:
      "The request resolves DNS, enters an edge POP, terminates or proxies TLS/L7 work, then reaches service ingress and application code.",
    ...
    insightSteps: [
      {
        ...
        description:
          "The request is not a static asset, so it cannot be answered only by a cache hit.",
      },
      {
        title: "DNS chooses the nearest POP",
        description:
          "Resolution chooses the entry point before the HTTP request crosses application boundaries.",
      },
      {
        title: "Edge terminates protocol work",
        description:
          "The TLS/L7 gate appears at the edge because API routes often terminate, inspect, or proxy protocol boundaries there.",
      },
      {
        title: "Service ingress receives dynamic traffic",
        description:
          "Dynamic traffic crosses into origin infrastructure or a load balancer/service gateway instead of stopping at cache storage.",
      },
      {
        title: "App service produces the response",
        description:
          "The final hop shows why API latency depends on both network routing and application behavior.",
      },
      {
        title: "Response returns through the edge",
        description:
          "The API response travels back through the same delivery boundary to the client.",
      },
    ],
  },
  ...
  steps: [
    {
      ...
      description:
        "The request starts at the user and DNS resolves where traffic should enter the delivery network.",
    },
    {
    ...
    },
    {
      ...
      description:
        "A cache hit ends early; a miss continues through origin or application infrastructure.",
    },
    {
      title: "Route outcome changes latency",
      description:
        "The final hop sequence explains origin load, cache behavior, and user-facing response time.",
    },
  ],
  concepts: [
    {
      ...
      description:
        "A web request crosses resolution, edge routing, cache lookup, and sometimes origin infrastructure.",
      bullets: [
    ...
    },
    {
      ...
      description:
        "The same user action can produce very different system load depending on cache and edge decisions.",
      bullets: [
