...
export const distributedScenarios = [
  {
    ...
    summary:
      "A sequencer assigns one global log position, then replicas append, acknowledge, and commit the same entry.",
    leaderNodeIds: ["B"],
    phases: [
      {
        title: "Client submits E7",
        summary:
          "A sends an operation to the sequencer instead of asking every replica to choose an order.",
        activeEdges: [["A", "B"]],
        activeNodes: ["A", "B"],
        statuses: {
          A: "submit E7",
      ...
      },
      {
        ...
        summary:
          "Each replica appends E7 at slot 42; this local write is shown as self-pulsing state.",
        activeEdges: [],
        selfNodes: ["C", "D", "E", "F", "G"],
    ...
    insightSteps: [
      {
        ...
        description:
          "Node A sends E7 to the sequencer so replicas do not independently choose incompatible positions.",
      },
      {
      ...
      },
      {
        title: "Replicas append locally",
        description:
          "The local append is not a network message; each node updates its own durable log.",
      },
      {
        title: "Quorum acknowledgements return",
        description:
          "Enough acknowledgements prove the ordered entry survived on a quorum of replicas.",
      },
      {
        title: "Commit becomes visible",
        description:
          "The client and any lagging replica can learn the committed order after quorum convergence.",
      },
    ],
  },
  {
    ...
    summary:
      "Local events and messages advance counters so nodes can preserve happened-before relationships without physical time.",
    phases: [
      {
    ...
    insightSteps: [
      {
        ...
        description:
          "The first pulse is local state only: A increments its logical clock without a network message.",
      },
      {
        title: "A sends its timestamp to D",
        description:
          "The active edge carries A's clock value so D can learn the happened-before relationship.",
      },
      {
        title: "D merges the max clock",
        description:
          "D updates to a clock greater than the received timestamp, preserving causality.",
      },
      {
        title: "C and F exchange unrelated work",
        description:
          "The side link advances independently, so that chain is concurrent with A's chain.",
      },
      {
        title: "Only a partial order exists",
        description:
          "Lamport clocks prove happened-before, but they do not prove that unrelated events happened at the same real time.",
      },
    ],
  },
  {
    ...
    trigger: "Run vote",
    summary:
      "A faulty replica equivocates, honest nodes exchange evidence, and the quorum converges on one decision.",
    leaderNodeIds: ["B"],
    faultyNodeIds: ["G"],
    nodeRoles: {
      B: "Commander",
    ...
    phases: [
      ...
      },
      {
        ...
        summary:
          "The honest view contains enough matching X reports to isolate the conflicting value.",
        activeEdges: [
          ["C", "E"],
          ["E", "F"],
          ["C", "F"],
    ...
    insightSteps: [
      ...
      },
      {
        ...
        description:
          "G sends conflicting values to different honest replicas, which is the Byzantine behavior.",
      },
      {
        title: "Honest replicas echo evidence",
        description:
          "Cross-check links show replicas sharing what they heard instead of trusting one sender.",
      },
      {
        title: "Majority observes X",
        description:
          "The conflicting report becomes an outlier once honest replicas compare evidence.",
      },
      {
        title: "Quorum certificate returns",
        description:
          "The commander receives enough matching evidence to identify the supported value.",
      },
      {
        title: "Decision X is committed",
        description:
          "Honest replicas commit the quorum-supported value and exclude the faulty sender's equivocation.",
      },
    ],
  },
  ...
  steps: [
    {
      ...
      description:
        "The first phase introduces a client request, logical-clock tick, or commander proposal.",
    },
    {
      title: "Coordination message fans out",
      description:
        "Active links show sequencing, causal delivery, or conflicting Byzantine messages.",
    },
    {
      title: "Replicas update local belief",
      description:
        "Nodes append, merge clocks, or compare received values depending on the scenario.",
    },
    {
      title: "Convergence or conflict is resolved",
      description:
        "Acknowledgements, concurrency, or quorum rejection make the final distributed invariant visible.",
    },
  ],
  concepts: [
    {
    ...
    },
    {
      ...
      description:
        "Distributed correctness depends on whether messages are delayed, concurrent, or actively malicious.",
      bullets: [
