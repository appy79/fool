...
function cardClass(activePart: number, step: number, tone: Tone) {
  return cx(
    ...
    isActive(activePart, step) &&
      `scale-[1.04] ${toneClasses[tone].active} animate-pulse motion-reduce:animate-none`,
  ...
  return (
    <div
      className={cx(
        ...
        active ? "bg-amber-300 shadow-[0_0_12px_rgba(251,191,36,0.45)]" : "bg-stone-800",
      )}
    />
  );
}
        ...
        {parts.map((part, index) => (
          <div
            ...
            className={cx(
              ...
              index === activePart &&
                "scale-105 border-amber-200 bg-amber-300 text-stone-950 shadow-lg shadow-amber-400/30",
    ...
    <div className="absolute bottom-8 left-7 right-7 top-32 grid grid-cols-[1fr_1.15fr_1.45fr_1fr] gap-5">
      <div className="flex flex-col justify-center gap-3">
        <Card
          ...
          tone="slate"
          label="caller"
          title="no algorithm switch"
          body="The caller sends a normal request and avoids if/else chains for every behavior."
        />
        ...
        <Card
          ...
          body="The context reads request shape, feature flags, tenant, or config to pick a strategy key."
        />
      </div>
      ...
      <div className="flex flex-col justify-center">
        <Card
          ...
          body="A dependency-injected registry resolves a concrete class behind one callable contract."
        />
      </div>

      <div className="rounded-3xl border border-orange-900/50 bg-orange-950/20 p-4">
        <div className="mb-3 text-[0.62rem] font-black uppercase tracking-[0.22em] text-orange-200/80">
          Concrete Strategies
        </div>
            ...
            return (
              <div
                ...
                className={cx(
                  ...
                  chosen
                    ? "scale-[1.03] border-amber-200 bg-amber-300 text-stone-950 shadow-lg shadow-amber-400/30"
                    : activePart >= 2
                      ? "border-orange-400/30 bg-stone-950/80 text-orange-100"
                      : "border-stone-800 bg-stone-950/70 text-stone-600",
                )}
              >
      ...
      <div className="flex flex-col justify-center gap-3">
        <Card
          ...
          body="The context accepts the common result type regardless of the selected implementation."
        />
        ...
        <Card
          ...
          tone="emerald"
          label="result"
          title="stable caller contract"
          body="The caller receives one response shape and remains closed to algorithm churn."
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="absolute bottom-8 left-7 right-7 top-32 grid grid-cols-[1fr_1.35fr_1fr] gap-5">
      <div className="rounded-3xl border border-emerald-900/40 bg-emerald-950/15 p-4">
        <div className="text-[0.62rem] font-black uppercase tracking-[0.2em] text-emerald-200/75">
          Modern Boundary
        </div>
        <div className="mt-5 space-y-3">
          <Card
            ...
            label="target"
            title="request DTO"
            body="Client code calls the clean interface used by the current system."
          />
          <Card
            activePart={activePart}
            step={5}
            ...
            body="The response returns in the same modern contract."
          />
        </div>
      </div>

      <div className="rounded-3xl border border-amber-900/50 bg-amber-950/20 p-4">
        <div className="text-[0.62rem] font-black uppercase tracking-[0.2em] text-amber-200/80">
          Adapter Translation Pipeline
        </div>
        <div className="mt-4 grid gap-3">
          <Card
            ...
            tone="amber"
            label="boundary"
            title="implements target"
            body="The adapter is substitutable anywhere the target interface is expected."
          />
          <Card
            activePart={activePart}
            step={2}
            tone="cyan"
            label="map out"
            title="field + protocol mapping"
            body="Names, IDs, envelope shape, auth headers, and error expectations are transformed."
          />
          <Card
            activePart={activePart}
            step={4}
            tone="violet"
            label="map back"
            title="normalize legacy response"
            body="Legacy status, nulls, and errors become the target response model."
          />
        </div>
      </div>

      <div className="rounded-3xl border border-rose-900/50 bg-rose-950/20 p-4">
        <div className="text-[0.62rem] font-black uppercase tracking-[0.2em] text-rose-200/75">
          Legacy Boundary
        </div>
        <div className="mt-5 space-y-3">
          <Card
            ...
            tone="rose"
            label="legacy"
            title="SOAP/RPC payload"
            body="The incompatible request shape exists only behind the adapter."
          />
          <div
            className={cx(
              "rounded-2xl border p-3 text-center transition-all duration-500",
              activePart >= 3
                ? "border-rose-300 bg-rose-950/70 text-rose-100"
                : "border-stone-800 bg-stone-950 text-stone-600",
            )}
          >
            <div className="text-[0.55rem] font-black uppercase tracking-[0.18em] opacity-70">
    ...
    <div className="absolute bottom-8 left-7 right-7 top-32 grid grid-cols-[0.9fr_1.1fr_1.7fr_1fr] gap-5">
      <div className="flex flex-col justify-center gap-3">
        <Card
          ...
          label="publisher"
          title="domain event"
          body="Publisher creates one event describing what happened."
        />
        <Card
          activePart={activePart}
          step={1}
          ...
          label="publish"
          title="emit once"
          body="Publisher does not know which consumers exist."
        />
      </div>
      ...
      <div className="flex flex-col justify-center">
        <Card
          ...
          label="subject / bus"
          title="fan-out queue"
          body="The subject or event bus gives each subscriber an independent delivery lane."
        />
      </div>

      <div className="rounded-3xl border border-cyan-900/50 bg-cyan-950/15 p-4">
        <div className="mb-3 text-[0.62rem] font-black uppercase tracking-[0.2em] text-cyan-100/80">
          Subscriber Lanes
        </div>
        <div className="grid gap-3">
          {subscribers.map(([name, body]) => (
            <div
              ...
              className={cx(
                ...
                activePart >= 3
                  ? "border-cyan-300/60 bg-cyan-950/50 text-cyan-100"
                  : "border-stone-800 bg-stone-950/70 text-stone-600",
              )}
            >
              <div className="text-xs font-black uppercase tracking-wider">{name}</div>
              <div className="h-2 overflow-hidden rounded-full bg-stone-900">
                <div
                  className={cx(
                    ...
                    activePart >= 3 ? "w-full bg-cyan-300" : "w-0 bg-cyan-300",
                  )}
                />
              </div>
              <div
                className={cx(
                  "rounded-full border px-2 py-1 text-center text-[0.52rem] font-black uppercase",
                  activePart >= 4
      ...
      <div className="flex flex-col justify-center gap-3">
        <Card
          ...
          label="reliability"
          title="ack / retry"
          body="Consumers retry idempotently when side effects fail."
        />
        <Card
          activePart={activePart}
          step={5}
          tone="emerald"
          label="effects"
          title="eventual completion"
          body="Side effects settle independently from the publisher."
        />
      </div>
    </div>
  );
    ...
    <div className="absolute bottom-8 left-7 right-7 top-32 grid grid-cols-[1fr_1.55fr_1fr] gap-5">
      <div className="flex flex-col justify-center gap-3">
        <Card
          ...
          body="The service needs data from a dependency that may be slow or failing."
        />
        <Card
          activePart={activePart}
          step={1}
          tone="emerald"
          label="closed"
          title="pass-through calls"
          body="Closed state forwards calls and records failures, timeouts, and latency."
        />
      </div>

      <div className="rounded-3xl border border-violet-900/50 bg-violet-950/20 p-4">
        <div className="text-[0.62rem] font-black uppercase tracking-[0.2em] text-violet-100/80">
        ...
        <div className="mt-5 grid gap-2">
          {breakerRules.map((rule) => (
            <div
              key={rule.label}
              className={cx(
                "rounded-xl border px-3 py-2 text-[0.62rem] font-black uppercase tracking-wider transition-all duration-500",
                activePart >= rule.activeAt
                  ? "border-violet-300/50 bg-violet-950/70 text-violet-100"
                  : "border-stone-800 bg-stone-950 text-stone-600",
              )}
            >
              {rule.label}
      ...
      <div className="flex flex-col justify-center gap-3">
        <Card
          ...
          body="Failures accumulate before the breaker opens."
        />
        <Card
          activePart={activePart}
          step={4}
          ...
          body="Users get a controlled result while the dependency recovers."
        />
        <Card
          activePart={activePart}
          step={5}
          tone="amber"
          label="probe"
          title="one guarded call"
          body="A small test call decides whether traffic can resume."
        />
      </div>
    </div>
  );
...
function PatternImplementationView({
  patternId,
  activePart,
}: {
  patternId: string;
