  ...
  return (
    <figure className="relative">
      ...
      <button
        ...
        aria-label={
          isPinnedRevealed ? "Hide Foundation identity record" : "Reveal Foundation identity record"
        }
      ...
      >
        <div
          className={`${styles.portraitCardInner} ${isPortraitRevealed ? styles.portraitCardFlipped : ""}`}
        >
          <div className={`${styles.portraitCardFace} absolute inset-0`}>
            <span id={portraitHintId} className="sr-only">
              Press to reveal the Foundation identity record. Hover also previews the reverse side
              for pointer users.
            </span>
            <Image
            ...
            />
            <div
              className={`${styles.nightOverlay} absolute inset-0 transition`}
              aria-hidden="true"
            />
            <div
              className={`${styles.signalOverlay} pointer-events-none absolute left-0 right-0 transition`}
              aria-hidden="true"
            />
            <div
              className={`${styles.gridOverlay} absolute inset-0 transition`}
              aria-hidden="true"
            />
            {!hasViewedBack ? (
          ...
          <div
            id={identityRecordId}
            className={`${styles.portraitCardFace} ${styles.portraitCardBack} absolute inset-0`}
          >
            <div className={styles.accessCard}>
              ...
              <div className={styles.accessMetrics}>
                <p>
                  <span>id</span> terminus.engineer
                </p>
                <p>
                  <span>clearance</span> seldon.approved
                </p>
                <p>
                  <span>trace</span> non-random
                </p>
              </div>

              <div className={styles.accessFooter}>
                <p>Prime Radiant cross-reference located.</p>
                <p>Builds reliable systems amid imperial entropy.</p>
                <p>Bias: practical fixes over prophecy.</p>
              </div>
            </div>
          </div>
        </div>
