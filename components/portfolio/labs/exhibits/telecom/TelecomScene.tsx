    ...
    <LabSceneFrame className="p-0 shadow-inner shadow-cyan-950/40">
      ...
      >
        ...
        <div className="absolute right-[4%] top-[8%] flex items-end gap-2 opacity-35 pointer-events-none">
          {[0, 1, 2, 3, 4].map((bar) => (
            <span key={bar} className="w-2.5 rounded-full bg-emerald-400/20" style={{ height: 60 + bar * 16 }} />
          ))}
        </div>

        {scenario.id === "data-session" ? (
          <div className={`absolute left-[39%] top-[6%] rounded-full border border-amber-500/30 bg-amber-950/85 px-4 py-1 text-[0.68rem] font-black uppercase tracking-[0.2em] text-amber-100 transition shadow-lg ${activeIndex >= 3 ? "opacity-100 border-amber-400" : "opacity-35"}`}>
            Policy & Quota Core Active
          </div>
        ) : null}
        {scenario.id === "billing-aggregation" ? (
          <div className={`absolute left-[58%] top-[6%] rounded-full border border-emerald-500/30 bg-emerald-950/85 px-4 py-1 text-[0.68rem] font-black uppercase tracking-[0.2em] text-emerald-100 transition shadow-lg ${activeIndex >= 3 ? "opacity-100 border-emerald-400" : "opacity-35"}`}>
            Aggregation Batch Window Open
          </div>
        ) : null}
        {scenario.id === "service-integration" ? (
          <div className={`absolute left-[70%] top-[6%] rounded-full border border-indigo-500/30 bg-indigo-950/85 px-4 py-1 text-[0.68rem] font-black uppercase tracking-[0.2em] text-indigo-100 transition shadow-lg ${activeIndex >= 3 ? "opacity-100 border-indigo-400" : "opacity-35"}`}>
        ...
        <div className="absolute bottom-5 left-5 z-20 w-[320px] rounded-2xl border border-emerald-500/30 bg-stone-950/85 p-3.5 shadow-xl">
          ...
          <div className="text-[0.6rem] font-black uppercase tracking-[0.2em] text-emerald-400/70">Telecom Stage Telemetry</div>
          <div className="mt-1 font-mono text-[0.72rem] font-bold text-stone-100 leading-relaxed max-h-16 overflow-y-auto">{activeOutput}</div>
        </div>
        ...
        {telecomStages.map((stage) => {
          ...
          if (stage.id === "ue") {
            ...
            shapeClasses = "rounded-full border-2 border-dashed border-emerald-400 bg-emerald-950/40";
            prefix = "🌀 ";
          } else if (stage.id === "policy") {
            // Rules registry script document page
            shapeClasses = "rounded-tr-2xl rounded-bl-2xl border-l-4 border-l-amber-500 bg-[#2b1f15]/80";
            prefix = "📋 ";
            ...
            shapeClasses = "rounded-full border border-orange-500/40 bg-orange-950/30 h-11 px-4 flex items-center justify-center";
            prefix = "📦 ";
            ...
            shapeClasses = "rounded-xl border-t-[6px] border-t-yellow-500 border-x border-b border-yellow-900 bg-[#1f1610]/90";
            prefix = "💾 ";
          } else if (stage.id === "billing") {
            // Document with corner fold-over
            shapeClasses = "rounded-tr-2xl rounded-l border-r-4 border-b-2 border-r-violet-400 bg-[#1c1230]/90";
            prefix = "🧾 ";
          }
          ...
          return (
            ...
            >
              {active ? <span className="absolute inset-[-0.6rem] animate-ping rounded-2xl bg-amber-400/20 motion-reduce:animate-none" /> : null}
              ...
              >
                <div className="flex flex-col items-center justify-center">
                  <span className="text-[0.58rem] opacity-75 font-mono tracking-widest leading-none mb-0.5">{prefix}{nodeCode}</span>
