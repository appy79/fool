...
export default function ConcurrencyScene({
  ...
  const gridColsClass = 
    mode.phases.length === 3 ? "grid-cols-3" :
    mode.phases.length === 5 ? "grid-cols-5" : "grid-cols-6";
        ...
        <div className="relative mt-6 grid grid-cols-[1fr_13rem_13rem] gap-4">
          <div className="space-y-3">
            {Array.from({ length: 4 }).map((_, index) => {
              ...
              return (
                <div key={index} className={`grid grid-cols-[5rem_1fr_6rem] items-center gap-3 transition ${active ? "opacity-100" : "opacity-25"}`}>
                  ...
                  >
                    <div className="text-[0.5rem] font-bold text-slate-500 uppercase tracking-widest leading-none">Core</div>
                  ...
                  <div className={`text-[0.68rem] font-black uppercase tracking-wider transition-colors ${
                    tState.isWaiting 
                      ? "text-red-400" 
                      : tState.isCommitted 
                        ? "text-emerald-400" 
          ...
          >
            ...
            <div className="text-[0.62rem] font-black uppercase tracking-[0.2em] text-slate-500">{isSemaphore ? "Completed Work" : isMultiprocessing ? "Merged Result" : "Shared Register"}</div>
            <div className={`mt-3 text-5xl font-black font-mono tracking-tight transition-all ${counterConsistent ? "text-amber-300 drop-shadow-[0_0_8px_rgba(251,191,36,0.35)]" : "text-rose-400 drop-shadow-[0_0_8px_rgba(244,63,94,0.35)]"}`}>
              {visibleObservedCounter}
            </div>
        ...
        <div className="relative mt-4 gap-3">
          {isQueue ? (
            <div className="rounded-2xl border border-orange-500/40 bg-[#1f172a]/60 p-4 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
              <div>
                <div className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-slate-400">Memory Buffer (Work Queue)</div>
              ...
              <div>
                <div className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-slate-400">Single-Threaded Reducer</div>
