...
export default function ComplexityScene({ isRouteSearch, activeDepth }: ComplexitySceneProps) {
  ...
  const searchNodes = [
    { id: "root", parent: "", depth: 0, x: 620, y: 75, label: isRouteSearch ? "start A" : "formula", result: "start" },
    { id: "reject-1", parent: "root", depth: 1, x: 470, y: 155, label: isRouteSearch ? "B +12" : "x1=false", result: "reject" },
    { id: "keep-1", parent: "root", depth: 1, x: 740, y: 155, label: isRouteSearch ? "C +4" : "x1=true", result: "keep" },
    { id: "reject-2", parent: "keep-1", depth: 2, x: 620, y: 235, label: isRouteSearch ? "B +8" : "x2=false", result: "reject" },
    { id: "keep-2", parent: "keep-1", depth: 2, x: 820, y: 235, label: isRouteSearch ? "D +5" : "x2=true", result: "keep" },
    { id: "reject-3", parent: "keep-2", depth: 3, x: 715, y: 315, label: isRouteSearch ? "A loop" : "x3=false", result: "reject" },
    { id: "keep-3", parent: "keep-2", depth: 3, x: 840, y: 315, label: isRouteSearch ? "B +3" : "x3=true", result: "success" },
  ];
    ...
    <LabSceneFrame className="p-0 border-violet-950/40 shadow-inner">
      ...
      >
        ...
        <div className="absolute left-[5%] top-[11%] max-w-[270px] text-[0.68rem] font-bold text-slate-400 leading-normal">
          Both problems have compact witnesses. Search may branch explosively, while one proposed witness can be verified directly.
        </div>
        ...
        <div className="absolute left-[5%] top-[18%] grid w-[27%] gap-3">
          {choiceRows.map((choice, index) => {
            ...
            return (
              ...
              >
                <div className="text-[0.6rem] font-black uppercase tracking-[0.18em] text-slate-400">{choice.label}</div>
                <div className="mt-0.5 text-xs font-black font-mono leading-tight">{choice.value}</div>
                <div className={`text-[0.65rem] font-extrabold uppercase tracking-wider leading-none mt-0.5 ${isRouteSearch ? "text-cyan-200" : "text-emerald-200"}`}>{choice.detail}</div>
              </div>
            );
          })}
          <div className={`rounded-tr-2xl rounded-bl-2xl rounded-tl-sm rounded-br-sm border border-violet-800 bg-slate-950 p-2.5 text-[0.62rem] uppercase tracking-wider font-black text-center text-white transition ${activeDepth >= 3 ? "opacity-100" : "opacity-40"}`}>
            {isRouteSearch ? "🔍 O(n) verification: sum complete tour" : "✅ Polynomial verification: evaluate formula"}
          </div>
        </div>
        ...
        {searchNodes.map((node) => {
          ...
          return (
            <SceneConnector
              ...
              className={rejected ? "bg-red-500/30" : kept ? "bg-cyan-400 shadow-[0_0_12px_rgba(34,211,238,0.5)]" : "bg-stone-800/40"}
          ...
          <div className={`grid gap-2 ${isRouteSearch ? "grid-cols-4" : "grid-cols-3"}`}>
            {benefitChecks.map((check) => {
              ...
              return (
                ...
                >
                  <span className="font-sans font-bold text-[0.6rem] text-slate-400 mr-1">{check.label}:</span>
                  {check.value}
                </div>
              );
            })}
